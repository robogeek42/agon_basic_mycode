...
A very respectable demonstration of a Ray Tracing scene from "back-in-the-day" when we could only dream of our modern world with realistic 3d real-time rendered full colour gorgeosness that we are so used to now.
...
You can see a checkerboard floor receeding into the horizon with a graduated sky overhead. Floating in mid air are two perfectly mirrored balls, impossibly refelcting the scene around them.  It uses **just 4 colours** but uses a dithering effect to give a senve of depth to the world.

## The CODE
Sure, I hear you say - this is BASIC, how hard can it be to understand?  Well, it turns out, it is not so easy.  There is no "define sphere" function, no verticies or plane equation describing the floor, and certainly not a line of OpenGL in sight.
...
This should be fairly self explanitory, but I want to point our the following:
...
* VDU Commands also come from BBC BASIC and are used to send commnads to the display and sound processors - In this case, the ESP32 that is the video/sound coprocessor in the Agon machines.
...
* X being 0 means that the camera is positioned centrally horizontally. Y, our position in the vertical, is negative, which means we are looking sligthly upwards.  Z represents depth in the scene, and is positive. This is just convention, and you will note in later parts of the code that Z appears reversed. This is why.
...
As mentioned there are 320x240 pixels in this mode.  Conventially, 0,0 is at the top-left of the screen, and 320x240 is at the bottom right.
...
So that gives us the X and Y directions to fire the ray (stated as U and V), but we are in 3D so we also need the Z direction or depth - W.  The next line 100 calcualtes that using pythagoras. 
